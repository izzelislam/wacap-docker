...
class SQLiteStorageAdapter {
    ...
    constructor(baseDir = './sessions') {
        ...
        this.dbPath = (0, path_1.join)(baseDir, 'wacap.db');
    ...
    async saveMessage(sessionId, message) {
        ...
        const timestamp = typeof rawTs === "object" && rawTs?.toNumber
            ? rawTs.toNumber()
    ...
    `);
        stmt.run(messageId, sessionId, jid, JSON.stringify(message), timestamp, fromMe);
    }
    async getMessages(sessionId, jid, limit = 50) {
      ...
      LIMIT ?
    `);
        const rows = stmt.all(sessionId, jid, limit);
    ...
    `);
        ...
        stmt.run(id, sessionId, contact.id, contact.name || null, contact.notify || null, JSON.stringify(contact));
    }
